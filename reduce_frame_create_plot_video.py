...
import sys
import PyQt5
# download ffmpeg to be able to run this code

def column(matrix, i):
  return [row[i] for row in matrix]

vidcap = cv2.VideoCapture('60ff.MOV')
...
os.system('ffmpeg -r 30 -f image2 -s 1920x1080 -i %d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p output1.mp4')
...
while success :
  ...
  plt.plot(D, y)
  #plt.plot(D*coefficentd, y)
  plt.axvline(x = k, color = 'r', linestyle = '--')
  plt.savefig(str(count) + 'fig.png') # save each frame and plot plot on it
...
os.system('ffmpeg -r 30 -f image2 -s 1920x1080 -i %dfig.png -vcodec libx264 -crf 25  -pix_fmt yuv420p output2.mp4')